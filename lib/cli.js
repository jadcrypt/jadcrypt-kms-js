...
function encrypt(argv) {
  const jadcrypt = new JadcryptKMS({ region: argv.region, encoding: argv.encoding });
...
const encodingOpts = builder =>
  builder.alias('e', 'encoding').string('e').describe('e', 'byte encoding').default('e', 'hex');

const demand = keys => builder =>
  builder.demand(keys);

const globalOpts = compose(
  usageOpts('<command>', '<data>'), regionOpts, keyIdOpts, contextOpts, encodingOpts, demand(['r'])
);

const encryptOpts = compose(
  usageOpts('encrypt', '<plain>'), regionOpts, keyIdOpts, contextOpts, encodingOpts, demand(['r', 'k'])
);

const decryptOpts = compose(
  usageOpts('decrypt', '<encrypted>'), regionOpts, contextOpts, encodingOpts, demand(['r'])
);

// eslint-disable-next-line no-unused-expressions
globalOpts(yargs.version().help())
